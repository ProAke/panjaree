...
// if(isset($_COOKIE['doc_member']) && $_COOKIE['doc_member']!=''){
// 	/*$ck = json_decode(base64_decode($_COOKIE['doc_member']));*/
// 	$ck = unserialize(base64_decode($_COOKIE['doc_member']));
// 	if($ck->id >0 && $ck->email!=''){
...
/*	$tpl->assignInclude( "INC_WGLOGIN", "../html/_inc_login.tpl" );
...
/*		$tpl->assignInclude( "INC_WGLOGIN", "../html/_inc_wgdetail.tpl" );
		...
		/*$tpl->prepare();*/
		//$tpl->assign("_ROOT.CHECK_SIGNIN","<a href='signout'>Sign out</a>");
		$tpl->assign("_ROOT.CHECK_SIGNIN","<a href='profile' style='font-size: 10px;line-height: 12px;padding:0 5px;'>Welcome, ".$data['auth_name']."<br>".$data['auth_lname']."</a>");

		$tpl->assign("_ROOT.CHECK_PROFILE","<li><a href='signout' style='font-size: 18px;padding:3px 5px;'><span class='glyphicon glyphicon-off' aria-hidden='true'></span></a></li>");
...
*/			
		...
		/*$tpl->prepare();*/
		$tpl->assign("_ROOT.CHECK_SIGNIN","<a href='signin'>Sign in</a>");	
	}
