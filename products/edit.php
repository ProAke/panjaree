...
if ($_POST['action'] == "save") {
    ...
    if ($_FILES["product_photo"]["size"] > "0") {
        ...
        $sql = "UPDATE `$tableProducts` SET `product_photo`='" . $new_name . "', `product_description`='" . $_POST['product_description'] . "' WHERE `id`=" . $_POST['id'];
        $conn->query($sql);
    } else {
        $sql = "UPDATE `$tableProducts` SET `product_description`='" . $_POST['product_description'] . "' WHERE `id`=" . $_POST['id'];
        $conn->query($sql);
    }
